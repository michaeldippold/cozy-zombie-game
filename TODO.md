...
- **Current milestone:** 13 (Hunger and day/night) complete (2026-09-14). Seven-node neighbourhood from milestone 12, plus hunger and a day/night clock.
- **Last completed task:** Milestone 13 acceptance, scripted. Hunger drains to zero over 10 minutes with no food; eating restores hunger and health together; starvation drains health and ends the game with its own overlay ("You starved to death."). The clock cycles day → dusk → night → dawn correctly (verified brightness and phase at each boundary). At night the room darkens and both sides of a window edge (house and yard) show a warm glow. A yard zombie was pulled toward a lit window during a paused-clock night test.
- **Previously:** Milestone 12 acceptance. Three park chasers followed the player park → street → blue house → upstairs, arriving one by one. Two pistol shots in the house pull about a third of the neighbourhood over 150 s, mostly from the yard. A calm house sees at most one visitor in five minutes. Trickle restocked the street, yard, and park four times in six minutes, never next to the player. Decorative upstairs windows are inert. Also that session: melee costs stamina (winded swings are half damage), "Remove boards" refunds planks, browser context menu suppressed on all game DOM. Before that: right-click context menus with auto-walk, screen-space gun hit test, doorway thresholds. Before that, milestone 11.
- **Blocked on:** nothing
- **Candidate next steps (not started, need Michael's call):** hands-on tuning of hunger drain rate, day length, and night pull strength — all chosen to be provably present, not tuned for feel; save/load, which matters more now that sessions span a day/night cycle; door open/close; weapon degradation; a "peek" through windows; Tiled importer once hand-written JSON nodes get tedious; real art last.
